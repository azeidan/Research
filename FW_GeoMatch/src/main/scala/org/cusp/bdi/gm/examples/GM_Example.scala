...
import org.cusp.bdi.gm.geom._
import org.cusp.bdi.util.{Arguments_QueryType, CLArgsParser, Helper, InputFileParsers}
...
object GM_Example extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val path = new Path(clArgs.getParamValueString(GM_Arguments.outDir))
    if (hdfs.exists(path))
      ...
      sparkContext.textFile(fileName)
        ...
        .mapPartitions(_.map(parts => {
          ...
          val arrCoords = parts._2.map(coords => {
            ...
            val x = coords._1.substring(0, coords._1.indexOf('.')).toInt + 1.0 // ceil
            val y = coords._2.substring(0, coords._2.indexOf('.')).toInt + 1.0 // ceil
