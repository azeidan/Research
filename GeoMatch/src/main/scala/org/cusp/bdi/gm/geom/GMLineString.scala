...
/**
  ...
  */
case class GMLineString(_payload: String, _coordArr: Array[(Double, Double)]) extends GMGeomBase(_payload, _coordArr) {

    override def toJTS(jtsGeomFact: GeometryFactory) = {

        var xyStart = coordArr(0)

        (1 until coordArr.length).map(i => {

            var xyEnd = coordArr(i)

            var lineStr = jtsGeomFact.createLineString(Array(new Coordinate(xyStart._1.toDouble, xyStart._2.toDouble), new Coordinate(xyEnd._1.toDouble, xyEnd._2.toDouble)))
