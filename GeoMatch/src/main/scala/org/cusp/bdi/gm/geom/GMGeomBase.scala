...
/**
  ...
  */
abstract class GMGeomBase(_payload: String, _coordArr: Array[(Double, Double)]) extends Serializable with Ordered[GMGeomBase] {
    ...
    protected def computeHilberXY(hilbertBoxWidth: Int, hilbertBoxHeight: Int, searchGridMinX: Int, searchGridMinY: Int, pointCoord: (Double, Double)) =
        (math.floor((pointCoord._1.toDouble - searchGridMinX) / hilbertBoxWidth).toInt, math.floor((pointCoord._2.toDouble - searchGridMinY) / hilbertBoxHeight).toInt)
} 