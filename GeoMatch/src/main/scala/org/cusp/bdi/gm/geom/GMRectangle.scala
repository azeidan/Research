...
/**
  ...
  */
case class GMRectangle(_payload: String, lowEnd: (Double, Double), highEnd: (Double, Double)) extends GMPolygonBase(_payload, Array(lowEnd, highEnd)) {
    ...
    override def toJTS(jtsGeomFact: GeometryFactory) =
        Seq(jtsGeomFact.createPolygon(closedRingCoord.map(coord => new Coordinate(coord._1.toDouble, coord._2.toDouble))))
