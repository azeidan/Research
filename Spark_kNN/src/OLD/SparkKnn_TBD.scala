...
//
//
//case class SparkKnn_TBD(debugMode: Boolean, spatialIndexType: SupportedSpatialIndexes.Value, rddLeft: RDD[Point], rddRight: RDD[Point], k: Int /*, gridWidth: Int*/) extends Serializable {
//
...
//        .mapPartitions(_.map(row =>(row._1._1, row._1._2, row._2)
////          if (row._2 <= coreObjCapacity._1)
////            Iterator((row._1._1.toFloat, row._1._2.toFloat, row._2))
////          else {
////
////            // split box on its Y-axis
////
////            var rowTemp = (row._1._1.toFloat, row._1._2.toFloat, row._2)
////
////            (0 to (rowTemp._3 / coreObjCapacity._1).toInt) // 2 rounds minimum at this point
////              .map(_ => {
////
////                // scale Y-coord
////                val needCount = Helper.min(coreObjCapacity._1, rowTemp._3)
////                // startY + (<Needed Objects> / <Curr Count> * <height>)
////                val newEndY = rowTemp._1 + (needCount / rowTemp._3.toFloat) * (rowTemp._2 - rowTemp._1)
////
////                val ret = (rowTemp._1, newEndY, needCount)
////
////                rowTemp = (newEndY + 1e6f, rowTemp._2, rowTemp._3 - needCount)
////
////                ret
////              })
////          })
...
//
//          val newWeight = totalWeight + row._3
//
//          if (totalWeight == 0 || totalWeight > coreObjCapacity._1 || newWeight > coreObjCapacity._2) {
//
//            partCounter += 1
//            totalWeight = row._3
//            stackRangeInfo.push(new MBRInfo(row._1, row._2))
//          }
//          else {
//
...
//
//            if (row._2 < stackRangeInfo.top.bottom)
//              stackRangeInfo.top.bottom = row._2
//            else if (row._2 > stackRangeInfo.top.top)
//              stackRangeInfo.top.top = row._2
//          }
//
//          new Point(row._1, row._2, new GlobalIndexPointUserData(partCounter))
//        })
//
...
////          count += 1
//
//          SpatialIdxOperations.extractLstPartition(bvGlobalIndexRight.value, row._1, k).length
//        }).max)
//      })
//      .max
//
...
//
//    val coreObjCapacityRight = ((rowCountRight / numPartitionsPreferred).toLong, rowCountRight / numPartitionsMin)
//
