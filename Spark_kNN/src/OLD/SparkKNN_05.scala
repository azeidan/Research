...
object SparkKNN {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val mapCenterInfo = HashMap[Int, Array[Int]]() ++ sc.parallelize(arrKMModelCenters)
        ...
        val mapGlobalIndex = HashMap[Int, Int]()

        var partCounter = 0
        var partRowCount = 0L
        var currK = mapCenterInfo.map(_._1).min
        var lstCurrK_Nearest = mapCenterInfo.get(currK).get

        while (mapGlobalIndex.size < mapCenterInfo.size) {

            mapGlobalIndex.put(currK, partCounter)
            partRowCount = mapKandCounts.get(currK).get
            var i = 0
            while (partRowCount < rowsPerPartition && i < lstCurrK_Nearest.size) {

                if (mapGlobalIndex.get(lstCurrK_Nearest(i)) == None) {

                    val count = mapKandCounts.get(lstCurrK_Nearest(i)).get
                    mapGlobalIndex.put(lstCurrK_Nearest(i), partCounter)
                    partRowCount += count
                }
                i += 1
            }
            ...
            // next K
            //            i = 0
            while (i < lstCurrK_Nearest.size && mapGlobalIndex.get(lstCurrK_Nearest(i)) != None) i += 1
            if (i == lstCurrK_Nearest.size)
                currK = mapCenterInfo.filter(_._2.size > 0).head._2.head
            else
                currK = lstCurrK_Nearest(i)

            lstCurrK_Nearest = mapCenterInfo.get(currK).get
        ...
        //        mapGlobalIndex.foreach(x => println(">>\t%d\t%d".format(x._1, x._2)))
        ...
        val partitionerByK = new Partitioner() {
            ...
            override def getPartition(key: Any): Int =
                key match {
                    case keyBase: KeyBase => mapGlobalIndex.get(keyBase.k).get
                    case _ => key.asInstanceOf[(Int, Double)]._1
                }
        }

        val rddDS1 = rddDS1Plain
            .mapPartitionsWithIndex((pIdx, iter) => iter.map(row => genKeyGeom(row, true, arrKMModelCenters, mapGlobalIndex)))
        ...
        val rddDS2 = rddDS2Plain
            .mapPartitionsWithIndex((pIdx, iter) => {
                ...
                iter.map(row => genKeyGeom(lineParser(row), false, arrKMModelCenters, mapGlobalIndex))
            })
            .partitionBy(partitionerByK)
