...
object SparkKNN {
    ...
    def main(args: Array[String]): Unit = {
        ...
        var rddDS1_XY = rddDS1Plain
            .mapPartitions(_.map(row => ((row._2._1 /*% boxWidth*/ ), (row._2._2 /*% boxWidth*/ )) /*, 1L)*/ ))
        ...
        var mapGlobalIndex = rddDS1_XY.mapPartitions(_.map(row => (kmModel.predict(Vectors.dense(row._1.toDouble, row._2.toDouble)), (ListBuffer(row)))))
            ...
            .mapPartitions(iter => {
                ...
                val (midX, midY) = (maxX - minX, maxY - minY)
