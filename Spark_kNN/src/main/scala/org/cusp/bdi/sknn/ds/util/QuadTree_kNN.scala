...
class QuadTree_kNN(_boundary: Rectangle) extends QuadTree(_boundary) with SpatialIndex_kNN {
  ...
  def this(lowerLeft: (Double, Double), upperRight: (Double, Double)) {

    this(null)

    val pointHalfXY = new Geom2D(((upperRight._1 - lowerLeft._1) + 1) / 2.0, ((upperRight._2 - lowerLeft._2) + 1) / 2.0)

    this.boundary = Rectangle(new Geom2D(pointHalfXY.x + lowerLeft._1, pointHalfXY.y + lowerLeft._2), pointHalfXY)
  }
